 ...
 *
 */
public final class JsonSettingValueHelper {
	private static final Map< Class<?>, Translator> translators = new HashMap<Class<?>, Translator>( );
	private static final Translator stringTranslator = new JsonElementToStringTranslator( null ); // TODO: check about trim and null
	...
	static {
		...
		translators.put( String.class, stringTranslator );
	}
	...
	public static LoadedSetting generateValue( String theName, JsonElement theValue, String theDescription, boolean isSensitive, String theSource, Class<?> theType ) {
		...
		String stringValue = "";  
		try {
			stringValue = ( String )stringTranslator.translate( theValue );
		} catch( TranslationException e ) {
			throw new ConfigurationException( String.format( "'%s' had has a value that could not be translated into a string.", theName ), e );
		}
	...
	public static LoadedSetting generateMap( String theName, JsonElement theValue, String theDescription, boolean isSensitive, String theSource, Class<?> theKeyType, Class<?> theValueType ) {
		...
		Translator keyTranslator = SettingValueHelper.getTranslator( theKeyType ); // the key translator uses the standard string translators because json only supports strings for key types
