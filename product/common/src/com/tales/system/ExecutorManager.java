...
public class ExecutorManager implements Facility {
	...
	private List<Executor> executorList			= Collections.unmodifiableList( new ArrayList<Executor>( 0 ) );
	private Map<String, Executor> executorMap	= Collections.unmodifiableMap( new HashMap<String, Executor>( 0 ) );
	...
	public Executor getExecutor( String theName ) {
		...
		return this.executorMap.get( theName );
	}
	...
	public <T extends Executor> T getExecutor( String theName, Class<T> theType ) {
		...
		try {
			Preconditions.checkArgument( !Strings.isNullOrEmpty( theName ), "need the name of the executor");
			return ( T )this.executorMap.get( theName );
	...
	public void register( String theName, Executor theExecutor ) {
		Preconditions.checkArgument( !Strings.isNullOrEmpty( theName ), "the executor needs a name" );
		Preconditions.checkNotNull( theExecutor );
		synchronized( this.lock ) {
			Preconditions.checkArgument( !this.executorMap.containsKey( theName ), String.format( "An interface named '%s' already exists.", theName ) );

			HashMap<String, Executor> newMap = new HashMap<>( this.executorMap );
			newMap.put( theName, theExecutor);
			executorMap = Collections.unmodifiableMap( newMap );
			
			List<Executor> newInterfaceList = new ArrayList<>( this.executorList );
			newInterfaceList.add( theExecutor );
			executorList = Collections.unmodifiableList( newInterfaceList );
		}
	}
}
